...
class Company:
    ...
    def get_free_cash_flow_forecast(self, parameter_list):
        ...
        #============= мой код =============
        free_cash_flow_list = df.loc['Taxes'] * (1 - df.loc['Capex Ratio'])
...
with st.beta_expander('Monte Carlo Simulation'):
    ...
    if inputs_radio == 'Search':
        ...
        for i in range(int(simulation_iterations)):
           ...
           # my code
        model_input_zzz = calculate_value_distribution(parameter_dict_1, parameter_dict_2, parameter_dict_distribution)
        ...
        ebit_list_of_lists_zzz.append(forecast_df_zzz.loc['EBIT'])
        model_output_zzz, equity_value_zzz = company.discount_free_cash_flows(model_input_zzz, discount_rate, terminal_growth)
